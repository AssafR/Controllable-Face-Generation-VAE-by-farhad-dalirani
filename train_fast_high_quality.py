#!/usr/bin/env python3
"""
Fast High Quality VAE Training Script
Balanced configuration for high quality with reasonable training time:
- Resolution: 128x128 (4x more pixels than 64x64)
- Parameters: 125M (vs 1.7B for ultra high quality)
- Training time: ~20 hours (vs 2000+ hours for ultra)
- Still uses perceptual loss for quality
"""
...
def train_fast_high_quality():
    """Train VAE with fast high quality settings."""
    
    print("🚀 Starting Fast High Quality VAE Training")
    print("=" * 60)
    print("Configuration:")
    ...
    # Load fast high quality configuration
    ...
    log_dir = f"runs/fast_high_quality_{timestamp}"
    ...
    patience_counter = 0
    early_stopping_patience = 15  # Reduced from 20 for faster training
    ...
    for epoch in range(config['max_epoch']):
        ...
        if val_loss < best_val_loss:
            ...
            # Save model
            model_path = f"{config['model_save_path']}/vae_fast_high_quality.pth"
        ...
        if (epoch + 1) % 20 == 0:
            ...
            with torch.no_grad():
                ...
                # Save sample images
                sample_path = f"fast_high_quality_samples_epoch_{epoch+1}.png"
    ...
    print(f"  • Model saved: {config['model_save_path']}/vae_fast_high_quality.pth")
    if writer is not None:
    ...
    with torch.no_grad():
        ...
        # Save final samples
        final_path = "fast_high_quality_final_samples.png"
    ...
    if writer is not None:
        writer.close()
    print(f"\n✅ Fast high quality training complete!")

if __name__ == "__main__":
    train_fast_high_quality()
