#!/usr/bin/env python3
"""
Ultra High Quality VAE Training Script
Combines the best settings for maximum quality:
- High resolution (256x256)
- Large latent space (1024)
...
def train_ultra_high_quality():
    """Train VAE with ultra high quality settings."""
    
    print("🚀 Starting Ultra High Quality VAE Training")
    print("=" * 60)
    print("Configuration:")
    print("  • Resolution: 256x256")
    print("  • Latent Space: 1024")
    print("  • Loss: MSE + L1 + Perceptual + LPIPS")
    print("  • Epochs: 200")
    print("  • Batch Size: 64")
    print("  • Learning Rate: 0.00005")
    print("  • Dataset: Full CelebA")
    print("=" * 60)
    
    # Load ultra high quality configuration
    ...
    print(f"  • Loss weights: MSE={loss_config.get('mse_weight', 0)}, L1={loss_config.get('l1_weight', 0)}, Perceptual={loss_config.get('perceptual_weight', 0)}, LPIPS={loss_config.get('lpips_weight', 0)}")
    ...
    log_dir = f"runs/ultra_high_quality_{timestamp}"
    ...
    for epoch in range(config['max_epoch']):
        ...
        for batch_idx, images in enumerate(train_pbar):
            ...
            train_lpips += loss_dict.get('lpips', 0)
        ...
        with torch.no_grad():
            ...
            for batch_idx, images in enumerate(val_pbar):
                ...
                val_lpips += loss_dict.get('lpips', 0)
        ...
        if writer is not None:
            ...
            writer.add_scalar('LPIPS/Train', train_lpips, epoch)
            writer.add_scalar('LPIPS/Val', val_lpips, epoch)
            writer.add_scalar('Learning_Rate', current_lr, epoch)
        
        # Print epoch summary
        print(f"\nEpoch {epoch+1}/{config['max_epoch']} Summary:")
        print(f"  Train Loss: {train_loss:.6f} (MSE: {train_mse:.6f}, KL: {train_kl:.6f}, L1: {train_l1:.6f}, Perceptual: {train_perceptual:.6f}, LPIPS: {train_lpips:.6f})")
        print(f"  Val Loss:   {val_loss:.6f} (MSE: {val_mse:.6f}, KL: {val_kl:.6f}, L1: {val_l1:.6f}, Perceptual: {val_perceptual:.6f}, LPIPS: {val_lpips:.6f})")
        ...
        if val_loss < best_val_loss:
            ...
            # Save model
            model_path = f"{config['model_save_path']}/vae_ultra_high_quality.pth"
        ...
        if (epoch + 1) % 10 == 0:
            ...
            with torch.no_grad():
                ...
                # Save sample images
                sample_path = f"ultra_high_quality_samples_epoch_{epoch+1}.png"
    ...
    print(f"  • Model saved: {config['model_save_path']}/vae_ultra_high_quality.pth")
    ...
    with torch.no_grad():
        ...
        # Save final samples
        final_path = "ultra_high_quality_final_samples.png"
    ...
    if writer is not None:
        writer.close()
    print(f"\n✅ Ultra high quality training complete!")

if __name__ == "__main__":
    train_ultra_high_quality()
